...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'recuperacao_clientes_quant_msg_range'
      AND conrelid = 'public.recuperacao_clientes'::regclass
  ) THEN
    ALTER TABLE public.recuperacao_clientes
      ADD CONSTRAINT recuperacao_clientes_quant_msg_range
      CHECK (quant_msg BETWEEN 1 AND 3);
  END IF;
END $$;

UPDATE public.recuperacao_clientes
SET
  imagem_1_url = COALESCE(imagem_1_url, imagem_url),
  imagem_1_ativa = COALESCE(imagem_1_ativa, imagem_ativa, false),
  legenda_1 = COALESCE(legenda_1, legenda),
  quant_msg = COALESCE(quant_msg, 1)
WHERE imagem_1_url IS NULL
   OR legenda_1 IS NULL
   OR quant_msg IS NULL;
